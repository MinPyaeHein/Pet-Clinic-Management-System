        ...
        <div class="col-12">
          <div class="card">
			...
			<th>Next</th>
                ...
                <tbody>
          <c:forEach var="at" items="${showAppointmentForm.doctorSchedules}" varStatus="s">
		    	<tr>
				<td>${s.index+1}</td>
				<td>${at.dayName}</td>
				<td>${at.startTime}</td>
				<td>${at.endTime}</td>
				<td>${at.appointments.size()}</td>
			    <c:choose>
                <c:when test="${at.appointments.size()==0}">
                <td><a href="#" class="btn btn-block btn-primary disabled">Next</a></td>
                </c:when>
                <c:when test="${at.appointments.size()>0}">
                <td><a href="showAppointmentPath2?frmScheduleId=${at.scheduleId}" class="btn btn-block btn-primary" >Next</a></td>
                </c:when>
                </c:choose>
				</tr>		
		        </c:forEach>
